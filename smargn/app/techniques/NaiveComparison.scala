...
object NaiveComparison {
  def run(word: String, inputDir: String, outputFile: String) = {
    ...
    val testedWord = formattedData.filter { case (w, o) => w == word
    }.first
    ...
    val similaritiesLocal = similarWordsDivision.collect().toList
    val wordOccurrences = testedWord._2


    //TODO finish display graph
    val firstLine = wordOccurrences.:\("word") { case (_, acc) => acc + "," + YearCnt.getNextYear
    }
    val toPrint = firstLine :: (testedWord :: similaritiesLocal).map { case (w, l) => w + l.:\("") { case (o, acc) => acc + "," + o}
    }
