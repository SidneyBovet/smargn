...
object NaiveComparisons {
   ...
   */
  def naiveDifferenceTopKScalingAverage(data: RDD[(String, Array[Double])], testedWord: (String, Array[Double]),
    ...
    val retrievedWords = retrieveTopK(k.toInt, naiveDifferenceMetricTopK, data, testedWord, order,
      List(acceptedDifference))
   ...
   * @return words that are similar
   */
  def naiveDivisionTopKScalingAverage(data: RDD[(String, Array[Double])], testedWord: (String, Array[Double]),
                                      parameters: List[Double]): RDD[(String)] = {
    val k = parameters.head
    val order = (x: (String, Double), y: (String, Double)) => if (x._2 != y._2) {
      x._2 < y._2
   ...
   * @param data collection of word, frequency to tuple to look into
   * @param testedWord word that we want to find its similar word
   * @param parameters L(0) contains the accepted difference between two array value that we accept, L(1) is the
   *                   shifting range and L(2) is the shifting step size
   * @return words that are similar
   */
  def naiveDifferenceScalingAverageWithShifting(data: RDD[(String, Array[Double])], testedWord: (String, Array[Double]),
                                                parameters: List[Double]): RDD[(String)] = {
    shift(data, testedWord, parameters, NaiveComparisons.naiveDivisionTopKScalingAverage, parameters(1).toInt,
      parameters(2).toInt)
  }
   ...
   * @param data collection of word, frequency to tuple to look into
   * @param testedWord word that we want to find its similar word
   * @param parameters L(0) contains the straightness of the curve that we accept, L(1) is the shifting range and L
   *                   (2) is the shifting step size
   * @return words that are similar
   */
  def naiveDivisionScalingMaxWithShifting(data: RDD[(String, Array[Double])], testedWord: (String, Array[Double]),
                                          parameters: List[Double]): RDD[(String)] = {
    shift(data, testedWord, parameters, NaiveComparisons.naiveDivisionTopKScalingAverage, parameters(1).toInt,
      parameters(2).toInt)
  }
   ...
   * @param parameters L(0) contains the accepted difference between two array value that we accept,
   * @return sum of differences of each element divided by size of the resulting array (filtered a priori by the
   *         accepted difference if too extreme value)
   */
  def naiveDifferenceMetricTopK(word1: Array[Double], word2: Array[Double],
                                parameters: List[Double] = List(15)): Double = {
    val acceptedDifference = parameters.head
    val zipped = proportionalScalarAverageSubstraction(word1).zip(proportionalScalarAverageSubstraction(word2))
   ...
   */
  def naiveDivisionMetricTopK(word1: Array[Double], word2: Array[Double], parameters: List[Double] = List()): Double = {
    val zipped = proportionalScalarAverageSubstraction(word1).zip(proportionalScalarAverageSubstraction(word2))
