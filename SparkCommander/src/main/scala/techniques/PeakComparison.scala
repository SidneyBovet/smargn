...
object PeakComparison {
  ...
  def peakComparisonWithMeanDerivative(data: RDD[(String, Array[Double])], testedWord: (String, Array[Double]),
    ...
    data.flatMap { x =>
      if (peakMeanDerivativeMetric(testedWord._2, x._2, List(windowSize, deltaY)) < proportionSimilarities) {
        List(x._1)
      } else {
        List()
      }
  ...
  def getPeaks(words: RDD[(String, Array[Double])],
    ...
    words.map(
      x => (x._1, testPeak(x._2, windowSize, deltaY).map(y => (y._1 + 1840, y._2 + 1840, y._3 + 1840)).sortBy(_._1)))
  ...
  def peakMeanDerivativeMetric(word1: Array[Double], word2: Array[Double],
    ...
    val deltaY = params(1)
    val distinctPeaks1 = testPeak(word1, windowSize, deltaY)
    val distinctPeaks2 = testPeak(word2, windowSize, deltaY)
  ...
  def testPeak(word: Array[Double], windowSize: Int, deltaY: Double): List[(Int, Double, Double)] = {
    val peaksLeft = peakMeanDerivative(word, windowSize, deltaY)
    val peaksRight = peakMeanDerivative(word.reverse, windowSize, deltaY)
   ...
   *         before any usage
   */
  def peakMeanDerivative(word: Array[Double], windowSize: Int, deltaY: Double = 3): List[(Int, Double, Double)] = {
  ...
  /** *******************************************************************************************************
    * Metrics that are not used at the time being
    * *******************************************************************************************************/
  /** *******************************************************************************************************
  ...
  /** *******************************************************************************************************
    ...
    * ******************************************************************************************************* */
  /**
   * Compares 2 words and outputs similarity. Uses windowPeakMinMax as definition for the peaks
   *
